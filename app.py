...
with tab1:
    ...
    for message in st.session_state.qa_history:
        qa_messages.chat_message(message["role"]).markdown(message["content"])
    if prompt_tab1:
        ...
        qa_messages.chat_message("user").markdown(prompt_tab1)
        qa_messages.chat_message("assistant").markdown(f'Echo: {prompt_tab1}')
...
with tab2:
    ...
    for message in st.session_state.sum_history:
        sum_messages.chat_message(message["role"]).markdown(message["content"])
    if prompt_tab2:
        ...
        sum_messages.chat_message("user").markdown(prompt_tab2)
        sum_messages.chat_message("assistant").markdown(f'Echo: {prompt_tab2}')
